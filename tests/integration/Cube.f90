...
!    TYPE (scalar)       :: temperature
    ...
    REAL(r8k), DIMENSION(n_x*n_y*n_z,1:3) :: vals
    ...
    CALL cube%setup (dims=(/ n_x, n_y, n_z /), x_coords=x_coords, y_coords=y_coords, z_coords=z_coords)
    DO i = 1, n_params_to_write
        IF (.NOT. ALLOCATED(vals_to_write(i)%attribute)) ALLOCATE(scalar::vals_to_write(i)%attribute)
        CALL vals_to_write(i)%attribute%setup (dataname(i), numcomp=1, values1d=vals(:,i))
    END DO
!    CALL

!    CALL vtk_legacy_write (unit, cube, pointdata=temperature, filename=filename, title=title)
